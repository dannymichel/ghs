...
async function setDefaultKeys(dryRun = false) {
    try {
        ...
        if (dryRun) {
            ...
            console.log(`Updated ~/.gitconfig with GPG key ${gpgKey} and SSH key ${sshKey}`);
        }
    } catch (error) {
...
async function deleteKeys(dryRun = false, hard = false) {
    try {
        ...
        if (dryRun) {
            ...
            if (hard) {
                // Delete GPG key
                execSync(`gpg --batch --yes --delete-secret-keys "${gpgKey}"`);
                execSync(`gpg --batch --yes --delete-keys "${gpgKey}"`);
