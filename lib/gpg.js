...
async function saveKeys(dryRun = false, configName = null, gpgKey = null, sshKey = null) {
    try {
        const config = loadConfig();

        if (!configName || !gpgKey || !sshKey) {
            const response = await promptSaveKeys();
            configName = configName || response.configName;
            gpgKey = gpgKey || response.gpgKey;
            sshKey = sshKey || response.sshKey;
        }
...
async function setDefaultKeys(dryRun = false, gpgKey = null, sshKey = null, userName = null, userEmail = null) {
    try {
        const config = loadConfig();

        if (!gpgKey || !sshKey || !userName || !userEmail) {
            const response = await promptDefaultKeys();
            gpgKey = gpgKey || response.gpgKey;
            sshKey = sshKey || response.sshKey;
...
async function createNewKeys(dryRun = false, configName = null, gpgName = null, gpgEmail = null, gpgPassphrase = null, sshKeyName = null) {
    try {
        const config = loadConfig();

        if (!configName || !gpgName || !gpgEmail || !gpgPassphrase || !sshKeyName) {
            const response = await promptNewKeys();
            configName = configName || response.configName;
            gpgName = gpgName || response.gpgName;
            gpgEmail = gpgEmail || response.gpgEmail;
            gpgPassphrase = gpgPassphrase || response.gpgPassphrase;
            sshKeyName = sshKeyName || response.sshKeyName;
        }
        ...
        if (dryRun) {
            ...
            const gpgKeyEscaped = `"${gpgName} <${gpgEmail}>"`;  // Escape the key name
...
async function deleteKeys(dryRun = false, hard = false, configName = null) {
    try {
        ...
        if (!configName) {
            const response = await promptDeleteKeys(config.key_pairs);
            configName = configName || response.configName;
        }
...
async function exportKeys(configName = null) {
    try {
        ...
        if (!configName) {
            const response = await promptExportKeys(config.key_pairs);
            configName = configName || response.configName;
        }
