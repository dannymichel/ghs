...
async function promptCommand() {
    ...
    return await inquirer.prompt({
        ...
        choices: ['analyze', 'save', 'new', 'delete', 'clone', 'switch', 'default']
    });
}
...
async function promptNewKeys() {
    ...
    return { configName, gpgName, gpgEmail, sshKeyName };
}

async function promptDeleteKeys() {
    const inquirer = await loadInquirer();
    const config = loadConfig();
    const configNames = Object.keys(config.key_pairs);

    return await inquirer.prompt([
        { type: 'list', name: 'configName', message: 'Select a configuration to delete:', choices: configNames }
    ]);
}

module.exports = { promptCommand, promptGpgKey, promptSshKey, promptClone, promptSwitch, promptDefaultKeys, promptSaveKeys, promptNewKeys, promptDeleteKeys };
